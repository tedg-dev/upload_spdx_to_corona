...
class CoronaAPIClient:
    ...
    def get_pat(self):
        ''' Get (all) Personal Access Tokens from Corona '''
        try:
            res_json = self.make_authenticated_request('GET', f'api/auth/pats')
            if not res_json['data']:
                raise CoronaError(f"Unexpected response structure while getting PATs")
            ...
            raise CoronaError(f"KeyError while getting PATs")
    ...
    def get_auth_token(self):
        ...
        if not self.token:
            try:
                ...
                msg = (f"Retrieved Corona PAT from Conjur: {self.pat}")
                logger.debug(msg)
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code in [429, 500, 502, 503, 504]:
                    msg = (f'Temporary server error ({response.status_code}). Retrying... ({attempt + 1}/{retries})')
                    logger.warning(msg)
    ...
    def _handle_error(self, e, response, action):
        ...
        message = error_messages.get(response.status_code, f'Error {action}: {response.status_code} ({response.text})')
